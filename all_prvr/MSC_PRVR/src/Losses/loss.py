...
class RKdAngle(nn.Module):
    ...
    def forward(self, student, teacher):
        ...
        # The original implementation materializes every N x N x N angle at
        # once.  MSRVTT supplies 20 captions per video, so a batch of 128 has
        # N=2560 and that tensor alone needs about 64 GiB.  Splitting only its
        # first axis preserves every original (i, j, k) term and its mean; no
        # captions or relations are sampled or dropped.  Checkpointing drops
        # each block's intermediates and recomputes it during backward.
        if self.chunk_size and student.size(0) > self.chunk_size:
            n_query = student.size(0)
            total_loss = student.new_zeros(())

            for start in range(0, n_query, self.chunk_size):
                end = min(start + self.chunk_size, n_query)

                def angle_block(student_block_input, teacher_block_input,
                                start=start, end=end):
                    with torch.no_grad():
                        td = (teacher_block_input.unsqueeze(0)
                              - teacher_block_input[start:end].unsqueeze(1))
                        norm_td = F.normalize(td, p=2, dim=2)
                        t_angle = torch.bmm(
                            norm_td, norm_td.transpose(1, 2))

                    sd = (student_block_input.unsqueeze(0)
                          - student_block_input[start:end].unsqueeze(1))
                    norm_sd = F.normalize(sd, p=2, dim=2)
                    s_angle = torch.bmm(
                        norm_sd, norm_sd.transpose(1, 2))
                    return F.smooth_l1_loss(
                        s_angle, t_angle, reduction='sum')

                total_loss = total_loss + checkpoint(
                    angle_block, student, teacher, use_reentrant=False)

            return total_loss / float(n_query ** 3)
...
class loss(nn.Module):
    def __init__(self, cfg):
        ...
        self.rkd_a_loss = RKdAngle(cfg.get('rkd_angle_chunk_size', 0))
