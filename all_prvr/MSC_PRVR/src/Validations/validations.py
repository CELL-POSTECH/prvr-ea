...
def get_gt_act(video_metas, query_metas):
    ...
    for vid_id in video_metas:
        v2t_gt.append([])
        for i, query_id in enumerate(query_metas):
            # ActivityNet caption IDs and our HDF5 video keys both retain
            # the `v_` prefix.  The original data release stored one side
            # without it, so accept either key representation.
            query_vid_id = query_id.split('#', 1)[0]
            if query_vid_id == vid_id or query_vid_id[2:] == vid_id:
    ...
    for vid_id in video_metas:
        ...
        for i, query_id in enumerate(query_metas):
            if collection == 'activitynet':
                # Our ActivityNet visual HDF5 keys retain the original `v_`
                # prefix, as do caption IDs.  The original release stripped
                # it from one side, so accept either representation here.
                query_vid_id = query_id.split('#', 1)[0]
                if query_vid_id == vid_id or query_vid_id[2:] == vid_id:
        ...
        # This is diagnostic logging only.  Some split/key combinations can
        # have no caption group for a video; do not make evaluation fail.
        if not clip_feat:
            continue
...
class validations(nn.Module):
    ...
    def forward(self, model, context_dataloader, query_eval_loader, logger):
        ...
        recorder = make_recorder(video_metas, 'clip', 'frame', self.cfg['clip_scale_w'], self.cfg['frame_scale_w'])
        if recorder is not None:
            recorder.add(score_clip, score_frame, score_sum, query_metas)
            recorder.close()
