"""Build BOA's MSRVTT SVAL artifact with the repository's original pipeline."""
import json
import os
...
def main():
    ...
    keywords = keywords_dict_construct(
        os.path.join(DATA_ROOT, "msrvtttrain.caption.txt"), glove_model)
    v_keywords = keywords_dict_construct(
        os.path.join(DATA_ROOT, "msrvttval.caption.txt"), glove_model)
