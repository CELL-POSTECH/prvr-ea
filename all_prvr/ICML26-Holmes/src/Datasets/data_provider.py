...
def _open_visual_feature(path):
    return BigFile(path) if os.path.isdir(path) else h5py.File(path, 'r')


def _read_video(feature, video_id, video2frames):
    if isinstance(feature, BigFile):
        return np.asarray([feature.read_one(frame_id) for frame_id in video2frames[video_id]])
...
class Dataset4PRVR(data.Dataset):
    ...
    def __init__(self, cap_file, visual_feat, text_feat_path, cfg, video2frames=None):
        ...
        self.visual_feat = _open_visual_feature(visual_feat)
    ...
    def __getitem__(self, index):
        ...
        frame_vecs = _read_video(self.visual_feat, video_id, self.video2frames)
