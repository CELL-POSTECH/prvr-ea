...
def collate_train(data):
    ...
    return dict(student_videos=student_videos,
                ...
                # Keep (batch, sequence, dim).  This is unchanged for the
                # original multi-token RoBERTa features and is required for
                # MSRVTT's one-token BERT features.
                student_text=student_target,
...
class Dataset4DLDKD(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for cap_id in cap_ids:
            ...
            cap_feat = np.atleast_2d(self.student_text_feat[cap_id][...])
            # Original RoBERTa payloads are token sequences.  MSRVTT's
            # supplied BERT payload is a single 1024-D token, so keep its
            # sequence axis instead of squeezing it into a 1-D vector.
            cap_tensor = torch.from_numpy(l2_normalize_np_array(cap_feat))[:self.max_desc_len]
            cap_tensors.append(cap_tensor)
            
            
            try:
                clip_cap_feat = np.atleast_2d(self.clip_text_feat[cap_id][...])
