...
def eval_q2m(scores, q2m_gts, use_multi_gt=None):
    # DreamPRVR also logs a text-to-text diagnostic.  Dense annotations define
    # query-to-video positives only, so retain that diagnostic's original
    # same-video-caption target rather than applying video indices to a
    # query-by-query score matrix.
    if use_multi_gt is None:
        use_multi_gt = multi_gt_enabled()
    if use_multi_gt:
...
class validations(nn.Module):
    ...
    def forward(self, model, context_dataloader, query_eval_loader,text_t2t=False):
        ...
        if os.environ.get('PRVR_STATIC_ANN_MODE'):
            ...
            return gmm_style(model=model, validator=self, context_loader=context_dataloader,
                             ...
                             method=os.environ.get('PRVR_STATIC_ANN_METHOD', 'DreamPRVR'),
                             batch_to_gpu=gpu)
    ...
    def compute_query2ctx_info(self, model, query_eval_loader, ctx_info, recorder=None):
        ...
        for idx, batch in tqdm(enumerate(query_eval_loader), desc="Computing q embedding", total=len(query_eval_loader)):
            ...
            if recorder is not None:
                recorder.add(_clip_scale_scores, _frame_scale_scores, _score_sum, batch[-1])
    ...
    def forward_text_recall(self, model, query_eval_loader,num_videos=10000,num_queries=40000):
        ...
        (t2t_r1, t2t_r5, t2t_r10, t2t_r100) = eval_q2m(
            -1 * text_sim_scores, t2t_gt, use_multi_gt=False)
