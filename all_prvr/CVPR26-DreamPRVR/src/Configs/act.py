# -*- coding: utf-8 -*-
import os
import yaml
from pathlib import Path

cfg = {}
_THIS_FILE = Path(__file__).resolve()
_PROJECT_ROOT = Path(os.environ.get("PRVR_PROJECT_ROOT", _THIS_FILE.parents[4]))


cfg["model_name"] = "DreamPRVR"

cfg['dataset_name'] = 'activitynet'
cfg['seed'] = 0

cfg['root'] = str(_THIS_FILE.parents[2])
cfg['data_root'] = os.environ.get("PRVR_DATA_ROOT", str(_PROJECT_ROOT / "datasets"))


cfg['visual_feature'] = 'i3d'
cfg['collection'] = 'activitynet'
cfg['map_size'] = 32
cfg['clip_scale_w'] = 0.6
cfg['frame_scale_w'] = 0.4



cfg['model_root'] = os.path.join(cfg['root'], 'DreamPRVR-CR-final', cfg['dataset_name'], cfg['model_name'])
cfg['ckpt_path'] = os.path.join(cfg['model_root'], 'ckpt')

# extra
cfg['sft_factor'] = 0.6


# dataset
cfg['num_workers'] = 24
cfg['no_core_driver'] = False
cfg['no_pin_memory'] = False
cfg['batchsize'] = 128
cfg['visual_feat_dim'] = 1024 

# opt
cfg['lr'] = 0.00025
cfg['lr_warmup_proportion'] = 0.01
cfg['wd'] = 0.01
cfg['margin'] = 0.2


# train
cfg['n_epoch'] = 100
cfg['max_es_cnt'] = 10
cfg['hard_negative_start_epoch'] = 20
cfg['hard_pool_size'] = 20
cfg['use_hard_negative'] = False
cfg['loss_factor'] = [0.02, 0.04, 0.003,0.01,1e-4,1e-3]
cfg['neg_factor'] = [0.2, 32, 1]

#deterministic intro query
cfg['deterministic_tau'] = 0.08


#diffusion
cfg['num_registers'] = 4
cfg['timesteps'] = 10
cfg['num_diffusion_layers'] =2


# eval
cfg['eval_query_bsz'] = 50
cfg['eval_context_bsz'] = 100


# model
cfg['max_desc_l'] = 30
cfg['max_ctx_l'] = 128
cfg['sub_feat_size'] = 768
cfg['q_feat_size'] = 1024
cfg['max_position_embeddings'] = 300
cfg['hidden_size'] = 384
cfg['n_heads'] = 4
cfg['input_drop'] = 0.31
cfg['drop'] = 0.31
cfg['initializer_range'] = 0.02
cfg['attention_num'] = 8


cfg['num_workers'] = 1 if cfg['no_core_driver'] else cfg['num_workers']
cfg['pin_memory'] = not cfg['no_pin_memory']


if not os.path.exists(cfg['model_root']):
    os.makedirs(cfg['model_root'], exist_ok=True)
if not os.path.exists(cfg['ckpt_path']):
    os.makedirs(cfg['ckpt_path'], exist_ok=True)


def get_cfg_defaults():
    return cfg
