...
def get_datasets(cfg):
    ...
    visual_feats = H5FrameFeature(os.path.join(rootpath, collection, 'FeatureData',
        'new_clip_vit_32_%s_vid_features.hdf5' % collection)) if cfg['feature_mode'] == 'clip' else BigFile(visual_feat_path)
    cfg['visual_feat_dim'] = visual_feats.ndims

    video2frames = read_dict(os.path.join(rootpath, collection, 'FeatureData',
        cfg['frame_index_feature'], 'video2frames.txt'))
    ...
    if os.environ.get('PRVR_MULTI_GT') and collection == 'tvr':
        test_caption_files['test'] = multi_gt_caption_file or os.path.join(rootpath, collection, 'tvrdenseval_v.caption.txt')

    test_text_feat_path = text_feature_path(rootpath, collection, cfg['feature_mode'])
    # Load visual features
    test_visual_feat_path = os.path.join(rootpath, collection, 'FeatureData', cfg['visual_feature'])

    test_visual_feats = H5FrameFeature(os.path.join(rootpath, collection, 'FeatureData',
        'new_clip_vit_32_%s_vid_features.hdf5' % collection)) if cfg['feature_mode'] == 'clip' else BigFile(test_visual_feat_path)
    test_video2frames = read_dict(os.path.join(rootpath, collection, 'FeatureData',
        cfg['frame_index_feature'], 'video2frames.txt'))


    if cfg['feature_mode'] == 'clip':
        test_visual_feats.set_frame_index_map(test_video2frames)
