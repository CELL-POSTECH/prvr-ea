...
def eval_by_task_type(moment_predictions, video2idx, ground_truth,
    ...
    for k, gt_item in tqdm(gt_by_desc_id.items(), desc="Loop over moments", leave=False):
        ...
        if len(gt_item["ts"]) == 1 and type(gt_item["ts"][0]) is list: # QV
            gt_item["ts"] = gt_item["ts"][0] # [st, ed]

            
        if len(gt_item["ts"]) >= 2 and type(gt_item["ts"][0]) is list:  # QV, List[List] didemo, fro all 3 splits, at least 4 ts for each, < 0.5% has more than 4.
            least_n_overlap = len(gt_item["ts"]) // 2  # True if overlapped with at least least_n_overlap GT ts.
...
def eval_retrieval(submission, ground_truth, iou_thds=(0.5, 0.7), recall_topks=(1,10,100), verbose=True, match_number=True, use_desc_type=False):
    ...
    for task_type in submitted_task_types:
        metrics, metrics_by_type = eval_by_task_type(
            ...
            iou_thds=iou_thds, recall_topks=recall_topks,
