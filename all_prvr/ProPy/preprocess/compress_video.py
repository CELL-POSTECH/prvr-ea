...
def prepare_input_output_pairs(input_root, output_root, dataset_name, ignore_exist=True):
    ...
    for root, dirs, files in os.walk(input_root):
        for file_name in tqdm(files, total=len(files)):
            if dataset_name == 'act': # mkv->mp4
                vid_name, fmt = file_name.split(".")
                # import pdb; pdb.set_trace() 
                if fmt != "mp4":
                    # print(file_name)
                    # print(file_name)
                    input_mkv_video_path = os.path.join(root, file_name)
                    input_mp4_video_path = os.path.join(root, f"{vid_name}.mp4")
                    if not os.path.exists(input_mp4_video_path):
                        ffmpeg.input(input_mkv_video_path).output(input_mp4_video_path).run()
                    file_name = input_mp4_video_path
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, required=True, choices=['act', 'cha', 'tvr', 'qv'])
    ...
    input_video_path_list, output_video_path_list = prepare_input_output_pairs(input_root, output_root, dataset_name=args.dataset)
