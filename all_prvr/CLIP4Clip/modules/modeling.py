...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        # Tight retrieval concatenates text and video tokens in CrossModel, so
        # it remains bound by that model's positional table. In loose meanP
        # retrieval the CrossModel and temporal position table are both unused;
        # retaining this assertion there would incorrectly reject otherwise
        # valid long raw-frame evaluations such as max_frames=128.
        loose_mean_pool = (
            check_attr('loose_type', self.task_config)
            and getattr(self.task_config, 'sim_header', 'meanP') == 'meanP'
        )
        if not loose_mean_pool:
