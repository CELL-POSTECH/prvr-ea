...
from dataloaders.dataloader_prvr_rawframes_retrieval import PRVRRawFramesRetrievalDataset
...
RAW_COLLECTION_BY_DATATYPE = {
    "raw_msrvtt": "msrvtt",
    "raw_activitynet": "activitynet",
    "raw_tvr": "tvr",
    "raw_charades": "charades",
}


def _raw_collection(args):
    return RAW_COLLECTION_BY_DATATYPE[args.datatype]


def dataloader_rawframes_train(args, tokenizer):
    dataset = PRVRRawFramesRetrievalDataset(
        collection=_raw_collection(args), subset="train", tokenizer=tokenizer,
        data_path=args.data_path, frame_root=args.frame_root, caption_root=args.caption_root,
        max_words=args.max_words, feature_framerate=args.feature_framerate,
        max_frames=args.max_frames, image_resolution=224,
        frame_order=args.train_frame_order, slice_framepos=args.slice_framepos,
        max_samples=args.max_train_samples,
    )
    train_sampler = torch.utils.data.distributed.DistributedSampler(dataset)
    dataloader = DataLoader(
        dataset, batch_size=args.batch_size // args.n_gpu, num_workers=args.num_thread_reader,
        pin_memory=False, shuffle=False, sampler=train_sampler, drop_last=True,
    )
    return dataloader, len(dataset), train_sampler


def dataloader_rawframes_test(args, tokenizer, subset="test"):
    dataset = PRVRRawFramesRetrievalDataset(
        collection=_raw_collection(args), subset=subset, tokenizer=tokenizer,
        data_path=args.data_path, frame_root=args.frame_root, caption_root=args.caption_root,
        max_words=args.max_words, feature_framerate=args.feature_framerate,
        max_frames=args.max_frames, image_resolution=224,
        frame_order=args.eval_frame_order, slice_framepos=args.slice_framepos,
...
for _datatype in RAW_COLLECTION_BY_DATATYPE:
    DATALOADER_DICT[_datatype] = {
        "train": dataloader_rawframes_train,
        "val": dataloader_rawframes_test,
        "test": dataloader_rawframes_test,
    }
