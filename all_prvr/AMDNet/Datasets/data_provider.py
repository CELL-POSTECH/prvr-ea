...
def sentence_pool(cap):
    """Return AMDNet's single-vector query input.

    CLIP features are already one vector per query.  Legacy RoBERTa/BERT
    features may be token sequences, so use mean pooling instead of taking an
    arbitrary first token.
    """
    return cap.mean(dim=0) if cap.ndim == 2 else cap
...
def collate_train(data):
    ...
    for index, cap in enumerate(merge_captions):
        target[index, :] = sentence_pool(cap)
...
class Dataset4PRVR(data.Dataset):
    ...
    def __init__(self, cap_file, clip_vid_feat_path, clip_text_feat_path, cfg, video2frames=None):
        ...
        with open(cap_file, 'r') as cap_reader:
            for line in cap_reader.readlines():
                ...
                if video_id not in self._video_id_set:
                    self.video_ids.append(video_id)
                    self._video_id_set.add(video_id)
        ...
        self.clip_vid_feat = h5py.File(self.clip_vid_feat_path, 'r') if isinstance(self.clip_vid_feat_path, str) else self.clip_vid_feat_path
    ...
    def __getitem__(self, index):
        ...
        clip_vecs = []
        if isinstance(self.clip_vid_feat, BigFile):
            clip_vecs = [self.clip_vid_feat.read_one(frame_id) for frame_id in self.video2frames[video_id]]
        else:
        ...
        for cap_id in cap_ids:
            clip_cap_feat = np.atleast_2d(self.clip_text_feat[cap_id][...]) # 17,1024 --> 512
...
class TxtDataSet4PRVR(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        cap_feat = np.atleast_2d(self.text_feat[cap_id][...])
