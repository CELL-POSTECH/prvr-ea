...
def get_datasets(cfg):
    ...
    visual_feat_path = (os.path.join(rootpath, collection, 'FeatureData',
                        'new_clip_vit_32_%s_vid_features.hdf5' % collection)
                        if cfg['feature_mode'] == 'clip' else
                        BigFile(os.path.join(rootpath, collection, 'FeatureData', cfg['visual_feature'])))


    video2frames = read_dict(os.path.join(rootpath, collection, 'FeatureData', cfg['frame_index_feature'], 'video2frames.txt'))
    ...
    test_text_feat_path = text_feat_path
    # Load visual features
    test_visual_feat_path = visual_feat_path

    test_video2frames = read_dict(os.path.join(rootpath, collection, 'FeatureData', cfg['frame_index_feature'], 'video2frames.txt'))
