...
from prvr_compat import H5FrameFeature, text_feature_path
...
class wrap_module(nn.Module):
    ...
    def forward(self, *inputs):
        return self.model.get_pred_from_raw_query(*inputs)
...
def eval_epoch(model, val_video_dataset, val_text_dataset, opt):
    ...
    if record_full is not None:
        record_full(video_metas, query_metas, query_context_scores, global_query_context_scores,
                    'clip', 'frame', opt.clip_scale_w, opt.frame_scale_w, score_sum)
    
    # this part is temporary. for visualization (case study use)
    # get similarity scores of queries
    # get_similarity(-1 * score_sum, t2v_gt, query_metas, video_metas)
    # return
...
def get_similarity(scores, t2v_gt, query_metas, video_metas, k=5):
    import pandas as pd
    ...
    rank_idx = rank_idx[:, :k]   # (#query=10895, k=5)
    ...
    row_idx = np.concatenate([[i] * k for i in range(len(query_metas))])
    top_scores = scores[row_idx, rank_idx.flatten()]
    top_scores = -1 * np.reshape(top_scores, (len(query_metas), -1))  # (#query, k)
    
    # pack into DataFrame. columns: top-5 scores (5 columns) , rank of GT (1 col)
    tmp_array = np.concatenate([top_scores, np.asarray(query_videos_idx)[:,None]], axis=1)
    _COL_NAME = ["first_score", "second_score", "third_score", "fourth_score", "fifth_score", "GT_rank"]
    new_df = pd.DataFrame(tmp_array, columns=_COL_NAME)
    convert_dict = {"first_score":float, 
                    "second_score":float,
                    "third_score":float,
                    "fourth_score":float,
                    "fifth_score":float,
    ...
    #new_df = new_df[new_df['GT_rank']<5]
    new_df["score"] = -1 * new_df.first_score.values[:] + new_df.second_score.values[:] + new_df.third_score.values[:] + \
                        new_df.fourth_score.values[:] + new_df.fifth_score.values[:]
    
    new_df = new_df.sort_values(by="score")
    new_df.to_csv('./sim_ranked.csv', index=True)
...
def get_top(scores, t2v_gt, query_metas, video_metas, max_indices, k=5):
    ...
    row_idx = np.concatenate([[i] * k for i in range(len(query_metas))])
    retrieve_clip_idx = max_indices[row_idx, rank_idx.flatten()] # (#query, k)
...
def setup_model(opt):
    ...
    # Checkpoints may have been created on a different physical GPU.  Load all
    # tensors onto the GPU selected for this evaluation, then move the model
    # normally below.  This does not change model weights or inference logic.
...
def start_inference(opt=None):
    ...
    text_feat_path = text_feature_path(rootpath, collection, opt.feature_mode)
    ...
    visual_feats = (H5FrameFeature(os.path.join(rootpath, collection, 'FeatureData',
                    'new_clip_vit_32_%s_vid_features.hdf5' % collection))
                    if opt.feature_mode == 'clip' else BigFile(visual_feat_path))
    ...
    if opt.feature_mode == 'clip':
        visual_feats.set_frame_index_map(video2frames)
