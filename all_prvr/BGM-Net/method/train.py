...
from prvr_compat import H5FrameFeature, text_feature_dim, text_feature_path
...
def train_epoch(model, train_loader, optimizer, opt, epoch_i, training=True):
    ...
    if opt.use_matcher_start_epoch != -1 and epoch_i >= opt.use_matcher_start_epoch:
        model.set_use_matcher(True)
    ...
    loss_meters = OrderedDict(clip_nce_loss=AverageMeter(), clip_trip_loss=AverageMeter(),
                      ...
                      clip_align_loss=AverageMeter(), frame_align_loss=AverageMeter(),
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader), desc="Training Iteration", total=num_training_examples):
        ...
        timer_start = time.time()
        # Tensor: (128, 32, 3072), (128, 102, 3072), (128, 102), (640, 28, 768), (640, 28), 
        # caption's sample index in the batch, list:(640)
        # loss_dict: 'loss_overall', 'clip_nce_loss', 'clip_trip_loss', 'frame_nce_loss', 'frame_trip_loss'
...
def train(model, train_dataset, val_video_dataset, val_text_dataset, opt):
    ...
    optimizer = BertAdam(optimizer_grouped_parameters, lr=opt.lr, weight_decay=opt.wd, warmup=opt.lr_warmup_proportion,
                         t_total=num_train_optimization_steps, schedule="warmup_cosine")#"warmup_linear")
    prev_best_score = 0.
    es_cnt = 0
...
def start_training(opt):
    ...
    collection = opt.collection     # name of dataset: 'tvr', etc.
    ...
    # text embedding file path: features extracted by Roberta
    text_feat_path = text_feature_path(rootpath, collection, opt.feature_mode)
    # Select only the supplied feature dimension.  The BGM-Net architecture
    # and its objectives are unchanged; this keeps its input projection aligned
    # with RoBERTa/BERT (dataset-dependent) or CLIP-B/32 (512-D).
    opt.q_feat_size = text_feature_dim(collection, opt.feature_mode)
    ...
    train_dataset = BasicDataset(caption_files['train'], visual_feats, text_feat_path, opt, video2frames=video2frames)

    val_text_dataset = TxtDataSet(caption_files['val'], text_feat_path, opt)

    val_video_ids_list = read_video_ids(caption_files['val'])
    val_video_dataset = VisDataSet(visual_feats, video2frames, opt, video_ids=val_video_ids_list)
    ...
    model_config = EDict(
        ...
        smp_rate=opt.smp_rate,  # sampling rate to control strides
        ...
        use_matcher=False)      # reset at each epoch
...
if __name__ == '__main__':
    ...
    set_seed(opt.seed)
    log_config(opt.results_dir, 'performance')      # also print configs
