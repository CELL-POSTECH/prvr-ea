...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        self.parser.add_argument("--output_root", type=str, default=None,
                                 help="absolute project results root (keeps checkpoints outside the dataset)")
        self.parser.add_argument("--feature_mode", choices=["resnet", "clip"], default="resnet")
        ...
        self.parser.add_argument("--multiGT", action="store_true", help="evaluate dense multi-positive ground truth")
        ...
        self.parser.add_argument("--use_matcher_start_epoch", type=int, default=0,
                                help="which epoch to use Hungarian Matcher to match query to clip,"
                                    "use -1 to disable")
        ...
        self.parser.add_argument("--max_ctx_l", type=int, default=128, help="max clip numbers of a video")
        ...
        self.parser.add_argument("--model_name", type=str, default='BGM_Net')
        ...
        self.parser.add_argument("--smp_rate", type=float, default=0.5)
    ...
    def parse(self):
        ...
        if isinstance(self, TestOptions):
            ...
            opt.model_dir = opt.model_dir if os.path.isabs(opt.model_dir) else os.path.join(opt.root_path, opt.collection,"results",opt.model_dir)
            ...
            result_parent = (os.path.join(opt.output_root, opt.feature_mode, opt.collection)
                             if opt.output_root else os.path.join(opt.root_path, opt.collection, "results"))
            opt.results_dir = os.path.join(result_parent, "-".join([opt.dset_name, opt.exp_id,
