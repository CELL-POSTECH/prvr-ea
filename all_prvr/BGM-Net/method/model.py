...
class BGM_Net(nn.Module):
    def __init__(self, config):
        ...
        # window_lens = [i for i in range(2, config.map_size + 1)]
        # strides = [int(math.ceil(i * config.smp_rate)) for i in window_lens]
        # self.pool_layers = nn.ModuleList([nn.Identity()]
        #                                  + [nn.AvgPool1d(l, stride=s) for (l, s) in zip(window_lens, strides)]
        #                                  )
